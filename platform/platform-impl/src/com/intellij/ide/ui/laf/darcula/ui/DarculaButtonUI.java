...
public class DarculaButtonUI extends BasicButtonUI {
  ...
  protected boolean paintDecorations(Graphics2D g, JComponent c) {
    ...
    if (UIUtil.isHelpButton(c)) {
      ...
      try {
        ...
        if (!c.hasFocus() && !isSmallComboButton(c) && c.isEnabled() && UIManager.getBoolean("Button.darcula.paintShadow") && !isToolbarButton(c)) {
  ...
  private Paint getBackground(JComponent c, Rectangle r) {
    ...
    } else if (isSmallComboButton(c)) {
      return JBColor.namedColor("Button.darcula.smallComboButtonBackground", UIUtil.getPanelBackground());
  ...
  protected Dimension getDarculaButtonSize(JComponent c, Dimension prefSize) {
    ...
    if (UIUtil.isHelpButton(c) || isSquare(c)) {
      ...
      int height = Math.max(prefSize.height, getMinimumHeight() + i.top + i.bottom) + getExtraVerticalInset(c);
